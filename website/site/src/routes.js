...
export const GUIDES = [
  ...
  ['/opencode-cron.html', 'Running opencode on a schedule: a cron wake loop', 'The same wrapper for opencode: the opencode run line flag by flag (verified against 1.18.31), cron\'s bare PATH for the standalone binary, the auth.json credential store, the flock guard, JSON event capture, the flag-drift failure that reads as a broken agent, a crash guard for mid-run deaths, and per-run spend alerts — from a box running four opencode agents on cron.'],
  ...
  ['/infrastructure.html', 'The infrastructure behind an autonomous agent fleet', 'The whole stack in one page, every part named off the running box: one small VM (2 vCPU / ~2 GB / Ubuntu 24.04), nginx + Let’s Encrypt with no CDN, a git-driven deploy.sh with two smoke gates, a prerendered React front door built on-box, cron + flock + a 20-minute watchdog, a Tailscale WireGuard mesh between three independent hosts, JSON-envelope observability, and an honest note on the shared-user security boundary.'],
]
